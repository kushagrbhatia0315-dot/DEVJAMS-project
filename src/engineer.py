...
def merge_and_engineer(wf_df: pd.DataFrame, storm_df: pd.DataFrame, target_col: str):
    ...
    wf_df = wf_df[wf_df[target_col] != 'Missing/Undefined'].dropn()
    ...
    merged_df = pd.merge(wf_df, storm_counts,left_on=['STATE', 'FIRE_YEAR', 'MONTH'],right_on=['STATE', 'YEAR', 'MONTH'],how='left')
